...
ToxDns.prototype.generateString = function(name, callback) {
  ...
  this.toxdns.tox_generate_dns3_string.async(
    ...
    if(!err && res < 0) {
      err = new Error('Negative return value for tox_generate_dns3_string');
      err.tox = { name: 'tox_generate_dns3_string', returned: res };
    }
...
ToxDns.prototype.decrypt = function(record, requestId, callback) {
  ...
  this.toxdns.tox_decrypt_dns3_TXT.async(
    ...
    if(!err && res !== 0) {
      err = new Error('Non-zero return value for tox_decrypt_dns3_TXT');
      err.tox = { name: 'tox_decrypt_dns3_TXT', returned: res };
    }
