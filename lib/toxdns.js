...
/**
 * Parse a ToxDns address, returning an object with two properties:
 * 'name' (before @) and 'domain' (after @). If not recognized as an
 * address, undefined is returned.
 * @private
 * @param {String} address - Address string to parse
 * @return {Object}
 */
ToxDns.prototype._parseAddress = function(address) {
...
/**
 * Resolves a toxdns address using toxdns v3. Assumes the domain
 * name in the given address uses the public key set in the ToxDns
 * constructor.
...
ToxDns.prototype.resolve = function(address, callback) {
  var toxdns = this,
      ...
      addr = this._parseAddress(address);
