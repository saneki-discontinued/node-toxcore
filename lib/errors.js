...
module.exports = {
  ...
  checkToxNewError: function(val) {
    if(val !== consts.TOX_ERR_NEW_OK) {
      throw (new ToxError('tox_new', val));
    }
  },
  ...
  bootstrap: function(val) {
    if(val !== consts.TOX_ERR_BOOTSTRAP_OK) {
      return (new ToxError('tox_bootstrap', val));
    }
  },
  ...
  unsuccessful: function() {
    return new ToxError('TOX_FUNC_RETURNED_UNCESSFULL', 0, 'api function returned unsuccessful, but error value indicates success');
  }
};
