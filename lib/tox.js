...
var FriendLosslessPacketCallback = ffi.Fuction('void', [ ToxPtr, 'uint32', UInt8Ptr, 'size_t', UserData ]);
...
Tox.prototype._initFriendLosslessPacketCb = function(){
  ...
  this._initCallback({
    ...
    wrapper: function(handle, friend, data, length, userdata){
      if(ref.address(data) !== 0) {
        data = new Buffer(ref.reinterpret(data, length)); // Copy to another Buffer
      } else {
        data = undefined;
      }
