...
var Tox = function(opts) {
  ...
  this._initNew(this._options, data);
  this._initCallbacks();
...
Tox.prototype.createLibrary = function(libpath) {
  ...
  return ffi.Library(libpath, {
    ...
    'tox_new':  [ ToxPtr, [ ToxOptionsPtr, UInt8Ptr, 'size_t', ref.refType(TOX_ERR_NEW) ] ],
 ...
 */
Tox.prototype.getFriendListSync = function(callback) {
  this._checkHandleSync();
 ...
 */
Tox.prototype.sendFileChunk = function(friendnum, filenum, position, data, callback) {
  this._checkHandleSync(callback);
  var eptr = ref.alloc(TOX_ERR_FILE_SEND_CHUNK);
...
Tox.prototype._initNew = function(options, data) {
  var size = size_t(0),
      eptr = ref.alloc(TOX_ERR_NEW);

  // If 'data' is a string, assume filepath to read data from
  // Todo: Asynchronous way to create a Tox from a filepath?
  //   ex: var tox = Tox.load({ data: '/etc/bot.tox' });
  if(_.isString(data)) {
  ...
  this._handle = this.getLibrary().tox_new(options, data, size, eptr);
