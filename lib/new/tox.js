...
Tox.prototype.addTCPRelay = function(address, port, publicKey, callback) {
  ...
  this.getLibrary().tox_add_tcp_relay.async(this.getHandle(), address, port, publicKey, eptr, function(err, res) {
    var terr = _this._getToxBootstrapError(eptr.deref());
    if(!err && terr) {
      err = terr;
    }
...
Tox.prototype.addTCPRelaySync = function(address, port, publicKey) {
  ...
  var success = this.getLibrary().tox_add_tcp_relay(this.getHandle(), address, port, publicKey, eptr);
...
Tox.prototype.setName = function(name, callback) {
  ...
  this.getLibrary().tox_self_set_name.async(
    ...
    var terr = _this._getToxSetInfoError(eptr.deref());
    if(!err && terr) err = terr;
    if(!err && !success) err = _this._getToxNotSuccessfulError();
    if(callback) {
      callback(err);
    }
  });
...
Tox.prototype.setNameSync = function(name) {
  ...
  var err = this._getToxSetInfoError(eptr.deref());
  if(err) throw err;
  if(!success) throw _this._getToxNotSuccessfulError();
};
 ...
 * @return {Error} Error, if any
 */
Tox.prototype._getToxSetInfoError = function(val) {
