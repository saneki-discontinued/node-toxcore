...
Tox.prototype.createLibrary = function(libpath) {
  ...
  return ffi.Library(libpath, {
    ...
    'tox_friend_by_public_key': [ 'uint32', [ _ToxPtr, _UInt8Ptr, ref.refType(TOX_ERR_FRIEND_BY_PUBLIC_KEY) ] ],
    'tox_friend_delete':   [ 'bool', [ _ToxPtr, 'uint32', ref.refType(TOX_ERR_FRIEND_DELETE) ] ],
    'tox_friend_exists':   [ 'bool', [ _ToxPtr, 'uint32' ] ],
    ...
    'tox_friend_get_public_key': [ 'uint32', [ _ToxPtr, 'uint32', _UInt8Ptr, ref.refType(TOX_ERR_FRIEND_GET_PUBLIC_KEY) ] ],
...
Tox.prototype.getFriendByPublicKey = function(publicKey, callback) {
  ...
  this.getLibrary().tox_friend_by_public_key.async(
    this.getHandle(), publicKey, eptr, function(err, friend) {
    var terr = errors.getToxFriendByPublicKeyError(eptr.deref());
    if(!err && terr) err = terr;
    if(callback) {
      callback(err, friend);
    }
...
Tox.prototype.getFriendByPublicKeySync = function(publicKey) {
  ...
  var err = errors.getToxFriendByPublicKeyError(eptr.deref());
  if(err) throw err;
  return friend;
};
...
Tox.prototype.getFriendPublicKey = function(friend, callback) {
  ...
  this.getLibrary().tox_friend_get_public_key.async(
    ...
    var terr = errors.getToxFriendGetPublicKeyError(eptr.deref());
    if(!err && terr) err = terr;
    if(callback) {
      callback(err, buffer);
    }
...
Tox.prototype.getFriendPublicKeySync = function(friend) {
  ...
  var err = errors.getToxFriendGetPublicKeyError(eptr.deref());
  if(err) throw err;
  if(!success) throw errors.getToxNotSuccessfulError();
  return buffer;
};
