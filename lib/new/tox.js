...
Tox.prototype.getAddress = function(callback) {
  ...
  var address = new Buffer(consts.TOX_FRIEND_ADDRESS_SIZE);
  this.getLibrary().tox_self_get_address.async(this.getHandle(), address, function(err) {
    if(callback) {
...
Tox.prototype.getAddressSync = function() {
  ...
  var address = new Buffer(consts.TOX_FRIEND_ADDRESS_SIZE);
  this.getLibrary().tox_self_get_address(this.getHandle(), address);
...
Tox.prototype.setName = function(name, callback) {
  ...
  if(_.isString(name)) name = new Buffer(name);

  this.getLibrary().tox_self_set_name.async(
    this.getHandle(), name, size_t(name.length), eptr, function(err, success) {
...
Tox.prototype.setNameSync = function(name) {
  ...
  if(_.isString(name)) name = new Buffer(name);
  var eptr = ref.alloc(_ToxErrorType),
      success = this.getLibrary().tox_self_set_name(this.getHandle(), name, size_t(name.length), eptr);
  var err = errors.getToxSetInfoError(eptr.deref());
  if(err) throw err;
...
Tox.prototype.getNameSize = function(callback) {
  if(!this._checkHandle(callback)) return;
  this.getLibrary().tox_self_get_name_size.async(this.getHandle(), function(err, size) {
...
Tox.prototype.getNameSizeSync = function() {
  this._checkHandleSync();
  var size = this.getLibrary().tox_self_get_name_size(this.getHandle());
  return Number(size);
};
...
Tox.prototype.getName = function(callback) {
  if(!this._checkHandle(callback)) return;
  var _this = this;
  this.getNameSize(function(err, size) {
    if(!err) {
      var name = new Buffer(size);
      _this.getLibrary().tox_self_get_name.async(_this.getHandle(), name, function(err) {
        if(callback) {
          callback(err, name.toString('utf8'));
        }
      });
    } else if(callback) {
      callback(err);
...
Tox.prototype.getNameSync = function() {
  this._checkHandleSync();
  var name = new Buffer(this.getNameSizeSync());
  this.getLibrary().tox_self_get_name(this.getHandle(), name);
