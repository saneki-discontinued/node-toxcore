...
ToxFriendManager.prototype._initEvents = function() {
  ...
  tox.on('friendName', function(e) {
    ...
    emitter.emit('name', e);
    emitter.emit('changed', new toxClientEvents.FriendChangedClientEvent(
      manager.get(e.friend()), 'name'
    ));
  });
  ...
  tox.on('friendStatusMessage', function(e) {
    ...
    emitter.emit('changed', new toxClientEvents.FriendChangedClientEvent(
      manager.get(e.friend()), 'statusMessage'
    ));
  });
  ...
  tox.on('friendConnectionStatus', function(e) {
    ...
    emitter.emit('changed', new toxClientEvents.FriendChangedClientEvent(
      manager.get(e.friend()), 'connectionStatus'
    ));
  });
