...
/**
 * Get or set this client's nospam value.
 * @param {Number} [newNospam] - New nospam value to set, if any
 * @return {Number} Nospam value
 */
ToxClient.prototype.nospam = function(newNospam) {
  // Could cache, but nospam isn't likely to be gotten/changed often
  if(arguments.length > 0) {
    this.tox().setNospamSync(newNospam);
  }
  return this.tox().getNospamSync();
};
